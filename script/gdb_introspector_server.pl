...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{GDB_INTROSPECTOR_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{GDB_INTROSPECTOR_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require GDB::Introspector;

    GDB::Introspector->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
gdb_introspector_server.pl - Catalyst Testserver

=head1 SYNOPSIS

gdb_introspector_server.pl [options]
